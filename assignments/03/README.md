...
- Finish the test program in file `t0.c` with a main function from which each of the functions in `t0img.c` are called (tested). Compile and execute your `t0img.c` (with stubs) and your test. Some test cases have already been completed for you, use those as examples :). You do NOT need to implement the test cases for tasks 01 and 02.
...
- OUTPUT:
    - If the `row` and `col` index is valid and `val` is non-`NULL`, set `*val` to `im->data[row][col]` and return `IMG_OK`. If `val` is `NULL`, return `IMG_BADINPUT`.
...
Compress these files into a zip file called `c.zip`; you can do this by using the following command from console:
```
$ make submit
```

i.e. you should have ONE file called `c.zip` containing the files listed above (you can open your `c.zip` file to verify you have the correct files in it).

Upload `c.zip` onto to the appropriate assignment submission page on the [CMPT 127 D100/D200/D300 Canvas page](https://canvas.sfu.ca/courses/62984) > Assignments.
